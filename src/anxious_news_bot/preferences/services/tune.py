...
class PreferenceTuningService:
    ...
    async def start_or_resume(
        ...
        try:
            ...
            candidate = QuestionnaireGenerationSchema.model_validate(raw, strict=True)
            if self._repetition_detector is None:
                self._quality.validate(
                    candidate, tuple(item.question for item in context.prior_answers)
                )
            else:
                self._repetition_detector.validate(candidate, context.prior_answers)
    ...
    async def _interpret_and_apply(self, questionnaire_id):
        for attempt in range(2):
            ...
            if self._duplicate_detector is not None:
                for change in proposal.changes:
                    if change.action == "create":
                        ...
                        resolution = await self._duplicate_detector.resolve(
                            change, candidates
                        )
            ...
            proposal = self._change_validator.validate(
                proposal, profile, questionnaire_id
            )
            try:
